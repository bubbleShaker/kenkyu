...
## 10/17 中間発表スライドを作成する
9月下旬に一応スライドを作成したが、かなり浅い部分しか書いていなかったのでより補強しようと思う。<br>
まず、スライドの構成が少し伝わりにくいなーと思う。相手の立場から考えると、いきなり問題を見るのではなく、
- カーディナリティ推定アルゴリズム→それによって解決できる問題

という流れの方がしっくりくる気がする。<br>
また、その後の基本的な考え方となる確率の紹介パートはそのままでよさそう。正直そこまで重要ではない気がしている(生の推定値がボロボロだったのを見たこともあり…)。<br>
少しスライドを作成すると、解決できる問題に対する言及があまりにも少ないことが分かる(DDos攻撃しか書いてない)。<br>
よく論文を読んでみると、3.PRACTICAL REQUIREMENTS FOR CARDINALITY ESTIMATIONについてあまりまとめられていないな、という印象を受けた。というかそれ以前の部分もあまりまとめられていない印象を受けた。<br>
今からこれらを見るのはあまりにも時間がかかるのでできないが、時間を見つけて一度まとめたいと思った。<br>
が、やっぱり少しだけまとめておこうと思う。

### Hll++「3.PRACTICAL REQUIREMENTS FOR CARDINALITY ESTIMATION」を見る
Hll++「3. カーディナリティ推定の実用的な要件 このセクションでは、PowerDrill \[9\]で使用するカーディナリティ推定アルゴリズムの要件を示します。我々は特定のニーズによって動かされましたが、これらの要求事項の多くはより一般的であり、他のアプリケーションにも同様に適用されます。」<br>
→読んでそのまま、カーディナリティ推定アルゴリズムに必要な要件をこれから示すらしい。<br>
Hll++「PowerDrill はカラム指向のデータストアであり、SQL ライクなクエリをバックエンドに送信するインターアクティブなグラフィカルユーザーインターフェースでもあります。カラムストアは高度にメモリ最適化されたデータ構造を使用しており、数千億行のデータセットに対して低レイテンシーでクエリを実行できます。このシステムは主にインメモリーキャッシュに依存しており、フロントエンドが生成するクエリーの種類に依存する程度は低い。典型的なクエリは、1つ以上のデータフィールドによってグループ化され、様々な基準によってフィルタリングされる。」<br>
→よくわからない。データベースの構造とクエリの関係性についてなんか言ってそうなのは分かるが…。<br>
Hll++「ほとんどのデータベースシステムと同様に、ユーザーはcount distinctクエリを発行することで、データセット内の別個の要素の数を数えることができる。多くの場合、このようなクエリは、あるフィールド（例えば、国や分）でグループ化され、別のフィールド（例えば、Google検索のクエリーテキスト）の別個の要素をカウントする。このため、1つのクエリで多数の異なるカウント計算が並列に実行される可能性がある。」<br>
→よくわからないが、要するに一つのクエリに対して複数のグループに並列にカウントをすることがあるらしい。<br>
Hll++「PowerDrillは平均して1日に約500万回の計算を行います。これらの計算の99%は100以下の結果をもたらします。これは、group-byクエリ内のいくつかのグループが少数の値しか持たないことがあり、必然的にカーディナリティが小さくなるという事実が一因である。一方、極端な例では、1日に約100回の計算で10億を超える結果が得られる。」<br>
→これもよくわからないが、1日に約500万回の計算を行っても、99%のグループではカーディナリティ数が100以下になるらしい。そんなことある？<br>
最後の、1日に約100回の計算で10億を超える結果が得られる、というのはよく分からない。なんで計算回数よりカーディナリティ数が大きくなってるの？<br>
Hll++「精度に関しては、ほとんどのユーザーは良い推定値で満足するでしょうが、非常に高い精度が要求される重要なユースケースもあります。特に、以前に実装されたMinCountアルゴリズム[2]の、ある閾値まではほぼ正確な結果を提供でき、その閾値を超えると近似的になるという特性を、ユーザーは高く評価しています。 したがって、カーディナリティ推定アルゴリズムに必要な主な要件は、以下のように要約できる：」<br>
→MinCountアルゴリズムについて全く見れていないので、明日の発表が終わったら見てみたい。<br>
- 正確さ。一定量のメモリに対して、アルゴリズムは可能な限り正確な推定を行うべきである。特にカーディナリティが小さい場合、結果はほぼ正確でなければならない。<br>
→少なくとも自前のHyperLogLogではn>=10000で崩壊していた。補正がうまくいっていなかったのか…？
- メモリ効率。アルゴリズムは利用可能なメモリを効率的に使用し、そのメモリ使用量をカーディナリティに適応させるべきである。つまり、推定するカーディナリティが非常に小さい場合、アルゴリズムはユーザが指定した最大メモリ量よりも少ないメモリを使用するべきである。<br>
→Hll++のスパース表現はこれを満たすためのものだったのだな、と思った。
- 大きなカーディナリティの推定。10億をはるかに超えるカーディナリティを持つ多集合は日常的に発生しており、このような大きなカーディナリティを合理的な精度で推定できることが重要である。<br>
→正直これはHll++でもまだ見れていないと思う。カーディナリティが80000くらいまでの誤差のグラフしか見れていない。
- 実用性。アルゴリズムが実装可能で保守可能であること。<br>
→これはプログラムの問題だが、Hll++に関しては保守性は高そう…？

これで一応3章は見終わったが、具体的な問題解決の話は出てこなかった。<br>
1,2章もざっと見るが、どうもDDosなどの単語が見当たらない。もしかしたらHyperLogLogの方に言及されていたかもしれない。正直そこまで見直す余裕がないので、ここは軽く触れるだけにしておいて先に進む。<br>
→結局pvの計算とddosだけにした。<br>

### スライド「確率的に求める場合」について
愚直に求める場合はO(n)だが、HyperLogLogでは…？ということが書かれていないのは不自然すぎる。なんで気にならなかったんだろう。<br>
→既存研究を紹介している2章を見ればよさそう。<br>

### 2章でのHyperLogLogへの言及
Hll++「HyperLogLogはFlajoletら\[7\]によって提案され、LogLogを改良したものである。前述の研究の後に発表された。相対誤差は1.04/ √ mであり、(ε, δ)モデルにおいてO(ε^-2 log log n+log n)の空間を必要とする。HyperLogLogは、順序統計に基づくアルゴリズムの中で最適に近いことが示されている。HyperLogLogの理論的特性は、MinCountや他の多くの実用的アルゴリズムよりも、与えられた固定量のメモリに対して優れた精度を持つことを証明している。HyperLogLogが改善するLogLog（小さな値のカーディナリティに対してはLinearCountingを使用）が、以前の実験的研究で非常に良い結果を出したという事実は、我々の選択を裏付けている。」<br>
→(ε, δ)モデルって何ですか…？<br>

### (ε, δ)モデルとは
2章の最初の方を見てみる。<br>
Hll++「FlajoletとMartinによる研究\[6\]を皮切りに、カーディナリティ推定問題に多くの再探索が費やされてきた。概要と利用可能なアルゴリズムの分類については[3, 14]を参照。カーディナリティ推定アルゴリズムの記述と分析には、2つの一般的なモデルがある： まず、データストリーミング(ε, δ)モデル[10, 11]は、{1, ... , n}のカーディナリティに対して、δの固定成功確率、例えばδ = 2/3で、(1±ε)近似を得るために必要な空間を分析する。 次に、推定量の標準誤差として定義される相対精度を分析することが可能である\[3\]。」<br>
→固定成功確率とは…？何が成功する確率…？(1±ε)近似ってなに…？必要な空間ってなに…？<br>
となったので、\[3,14\]を見てみる。

#### [3(A Statistical Analysis of ProbabilisticCounting Algorithms)](https://arxiv.org/pdf/0801.3552)を見る
>ABSTRACT 本稿では、データストリームアプリケーションにおけるカーディナリティ推定の問題を考察する。低次元のデータスケッチを形成するために擬似ランダム変量を使用する2つの手法に焦点を当て、確率的計数アルゴリズムの統計的分析を行う。従来の統計的手法を適用し、選択された順序統計量、またはランダム投影のいずれかを格納することに基づく確率的アルゴリズムを比較する。両者ともカーディナリティの推定量を導出し、最大項推定量は再帰的に計算可能であり、指数関数的に誤差が減少することを示す。さらに、両推定量が同程度の漸近効率を持つことを示し、2つのアプローチの間の予期せぬ関連性を示すことにより、この結果を説明する。

あまりわからないがとりあえず読み進めてみる。<br>
>1 はじめに 高スループットで、一時的に観測されるデータストリームは、コンピュータ科学者や統計学者にとって斬新で挑戦的な問題を提起する（Muthukrishnan, 2005; Aggarwal, 2007）。科学技術の進歩は、分析に利用可能なデータセットのサイズとデータ取得速度の両方を継続的に拡大している。その例として、ルータ上のインターネットトラフィックの増大(Akella et al., 2003; Cormode & Muthukrishnan, 2005b)、高頻度の金融取引、商用データベースアプリケーション(Whang et al., 1990)などが挙げられる。

これは現実問題へ焦点を当てた記述っぽい。<br>
>カーディナリティ、頻度モーメント、分位数、経験エントロピーなど、データストリームの特性をオンラインで近似することは、大きな関心を集めている（Cormode & Muthukrishnan, 2005a; Harvey et al.） その目的は、データのサブリニア表現を構築し、それを維持することであり、そこからターゲット特性を高効率で推論することができる（Aggarwal, 2007）。データ・ストリーム・アルゴリズムでは、通常、データを1回だけ通過させる、つまり、データを観測し、表現を更新するために処理し、その後破棄する。推論手順に関して「効率的」とは、推定値が高い確率で正確であることを意味する。データの取り扱いに関しては、アルゴリズムがデータ要素あたりの処理・更新時間が速く、ストレージの使用量が少なく、データの到着順序に影響されないことを意味する。これは、データの繰り返しパターンに敏感なサンプリングベースの手法とは対照的である。

よくわからないが、データを分析する上ではカーディナリティ以外にも、頻度モーメント、分位数、経験エントロピーなどの特性も重要らしい。<br>
「データ・ストリーム・アルゴリズム」というのはHyperLogLogなどもそうなのだろうか…？<br>

>この論文では、ストレージの制約により過去に記録された包括的なリストを保持することが不可能な場合に、データストリーム中の明確な項目の数を推定する問題に焦点を当てる。を推定する問題に焦点を当てる。に焦点を当てる。別個の項目の数またはカーディナリティとは、例えば、ある時間ウィンドウ内で観測された インターネットトラフィックの所与の時間ウィンドウ内で観測され、異常検知の目的で監視された、送信元-宛先IPアドレスの組を指すことができる。例えば、ネットワーク上のサービス拒否攻撃などの異常検知の目的で監視される、送信元-宛先IPアドレスのペアを指すことができる（Giroire, 2009）。コンピュータサイエンスの文献には、カーディナリティ推定に関する驚くほど長い研究の歴史がある。Flajolet＆Martin（1985）の先駆的な研究から始まり、主流の統計研究とは切り離されて発展してきた。とは切り離されて発展してきた。我々の目的は、これらのアルゴリズムを「伝統的な」統計学の用語で再分析することである。統計的用語で再分析することである。その焦点は、漸近的相対効率、枢要量、統計誤差の境界の観点からの比較にある。量、統計的誤差の境界での比較に焦点を当てる。とは対照的に、漸近的な相対的効率、極めて重要な量、統計的誤差の境界の観点からの比較に焦点を当てる。我々は、以下のようなハッシュ関数を利用したアルゴリズムのスケッチに集中する。

漸近的相対効率、枢要量などの比較はHyperLogLogなどでは間違いなく見ていない部分なので気になる。

>本稿の構成は以下の通りである。 第2節では、ハッシュ関数やハッシュなどの用語を定義し、カーディナリティ推定アルゴリズムの簡単で選択的な歴史を述べる。第3節では次数統計に基づく手法の最尤推定量（MLE）を導出し、第4節ではランダム投影を導出する。次統計法については、連続分布からサンプリングする場合、サンプリング分布の選択は重要ではないが、幾何分布からのサンプルを用いることで、漸近的な相対効率を大幅に低下させることなく、ストレージの大幅な節約が達成できることを示す。また、これらの推定量は指数関数的に減少する誤差境界で再帰的に計算可能であることを示す。次に、αがゼロに近いα安定分布を用いた射影法の近似推定量を提案する。セクション5では、2つの方法を比較し、ある意味では本質的に等価であることを予想外に発見する。最後にセクション6では、シミュレーションデータを用いて、我々のアルゴリズムの性能を既存のベンチマーク・アルゴリズムと比較する。第7節で本論文の結論を述べる。

すでに3節での最尤推定量から怪しい。見たい気持ちは山々だが、(ε, δ)モデルについて知りたかったはずなので、一旦置いておく。

#### サイトを調べてみた
[ε-δ論法（イプシロンデルタ論法）を理解する](https://qiita.com/nognog/items/83777ae1cd7c4e9fd0b7)という記事を発見する。<br>
\[14\]を見るのも良いが、それでもわからなかったらこれを見て、それでもわからなかったらとりあえず置いておこうと思う。

### 方向転換
(ε, δ)モデルについてはとりあえず置いておこうと思う。今日中に理解できることではないと思うので。<br>
HyperLogLogのバイアスについて少し深堀りしようかな、と思う。そしてそれにどのように対処しているのかや、改良版のHyperLogLog++ではどのように問題に対処しているのか、を話したいと思った。<br>
多分その方が自分の実験を絡めて話せると思うし、HyperLogLog++の理解も深められるし、既存研究の理解を深めるためにも良いと思うので…。<br>
本当は順番的にはLogLogがよいのだろうけど、解析が本当に無理なので、プログラムを作って適当に比較するくらいしかできないかもしれない。<br>

### HyperLogLogを精度14でデータ取り直し
Hll++ではp=14に統一していたので…。<br>
本来は論文と同じように各カーディナリティについて5000回データを取って分位数や中央値など取りたいが、それは追々やってみたい。<br>
また、カーディナリティに20000,40000,60000,80000も追加したい。これはHll++での実験と合わせたかったため。<br>
さらに、HllNoBiasを作る所以であったある閾値を境にした誤差の大きくなっていく様子も見てみたいが、今は時間がなさそう。<br>

#### 補正なしの場合
![photo](photo/241017/p=14_nhosei.png)<br>

#### 補正ありの場合
![photo](photo/241017/p=14_hosei.png)<br>

#### 40000くらいからおかしくね？
m=2^14より、5m/2=40960くらいからLINEARCOUNTINGを使われなくなる。<br>
だから補正以前の計算がおかしいのかなーという気持ちになってきた。<br>

#### とんでもない間違いを見つけた
rho関数がおそらく間違えていた。仮にp=14だとすると、wのビット長は32-14=18なのだが、ここを32で固定していた。仮に左から0パディングされるならばrhoの返り値が大きくなってしまうはず。<br>
→修正したので補正なしの場合から実験してみる。<br>

#### プログラム修正後、補正なしの場合
![photo](photo/241017/p=14_nhosei2.png)<br>

#### プログラム修正後、補正ありの場合
![photo](photo/241017/p=14_hosei2.png)<br>
カーディナリティが大きくなるにつれて誤差が小さくなっていそうなのが確認でき、めちゃくちゃ嬉しい。Hll++の論文通りの結果になっている。<br>

### 他にスライドに入れたいこと
あまり時間がないので微妙だが、整理もかねて他に加えたいことを挙げてみる。<br>
- HyperLogLogについて、2^32付近での補正なし、補正ありの実験をしてデータをまとめる<br>
→スペック的に無理。cardinality=10^7くらいでも数分かかったので…。最終的にKilledというエラー(?)が出たので諦め。
- HyperLogLogからHyperLogLog++で改良された点について軽くまとめる
→ちょっと厳しそうなので明日は諦めて、明日以降実験をしつつ読み進める。

これくらいかなーと思う。上から順番にやって、Hll++については時間の限り読み進めてまとめる感じになりそう。<br>
→cardinality=0の時のバイアス補正なし推定値が0.7mになることも入れておいた。<br>
式を見ればそうなることは当たり前だが、バグが起きてないかという意味で確認するのは良いことかなーと思った。<br>
